...
import { createCoreEditor } from './components/document/core-editor/core-editor.js';
...
async function init() {
  ...
  try {
    ...
    if (editorContainer) {
      editorInstance = await createCoreEditor(editorContainer, veltClient, user);

      // Wire up scroll container for sidebar scroll-to-heading
      if (editorInstance.scrollContainer) {
        documentCanvas.setScrollContainer(editorInstance.scrollContainer);
      }
