...
export function createMapStoreSidebar(container, { userName = 'S', onSelectStore } = {}) {
  ...
  function columnsIcon(color) {
    return `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="18" rx="1"/><rect x="14" y="3" width="7" height="18" rx="1"/></svg>`
  }
  ...
  function render() {
    ...
    const filtered = storeSections.map(s => ({
      ...s,
          ...
          ${filtered.map(section => `
            <div style="display:flex;flex-direction:column;gap:2px;">
              ...
              ${section.items.map(itemId => {
                ...
                const sel = itemId === selectedStoreId
                return `<button class="sidebar-list-item" data-id="${itemId}" style="display:flex;align-items:center;gap:8px;padding:0 8px;height:32px;border-radius:8px;border:none;cursor:pointer;text-align:left;background:${sel ? 'rgb(1,108,221)' : 'transparent'};font-family:Inter,sans-serif;">
                  ${columnsIcon(sel ? 'rgb(254,255,255)' : 'var(--task-text)')}
                  <span style="font-size:12px;color:${sel ? 'rgb(254,255,255)' : 'var(--task-text)'};">${item.name}</span>
                </button>`
              }).join('')}
            </div>
          `).join('')}
