...
// [Velt] Import initVelt from the Velt client SDK
import { initVelt } from '@veltdev/client';

// [Velt] API Key - Replace with your own from https://console.velt.dev
...
export async function initializeVelt() {
  ...
  try {
    // [Velt] Initialize the Velt SDK with API key
...
export async function authenticateUser(user) {
  ...
  try {
    ...
    // [Velt] Identify user with JWT token for secure authentication
    ...
    try {
      // [Velt] Fallback to simple identify without token (for demo purposes)
      await veltClient.identify(user);
...
export async function setVeltDocument(documentId, documentName = 'Untitled') {
  ...
  try {
    // [Velt] Set the document scope for collaboration
...
export function enableDarkMode() {
  if (veltClient) {
    // [Velt] Set dark mode on Velt components
...
export function setVeltDarkMode(isDark) {
  if (veltClient) {
    // [Velt] Sync dark mode with Velt components
...
export async function signOutUser() {
  if (veltClient) {
    try {
      // [Velt] Sign out the current user
...
export function subscribeToVeltInitState(callback) {
  ...
  // [Velt] Subscribe to SDK ready state
