...
export function useCurrentDocument(): CurrentDocument {
  ...
  useEffect(() => {
    ...
    if (docId) {
      ...
      setDocumentId(docId);
      localStorage.setItem('tiptap-suggestions-document-id', docId);
    } else {
      // 2. Check localStorage for existing document
      const stored = localStorage.getItem('tiptap-suggestions-document-id');
      if (stored) {
        docId = stored;
      } else {
  ...
  return useMemo(
    () => ({
      ...
      documentName: "Tiptap Suggestions",
    }),
    [documentId]
  );
}
