...
const nextConfig = {
  transpilePackages: ['@veltdev/react', '@veltdev/plate-crdt-react', '@veltdev/plate-comments-react', '@slate-yjs/core', '@slate-yjs/react'],
  webpack: (config) => {
    ...
    config.resolve.alias = {
      ...
      // Ensure single instances of Slate packages (prevents duplicate context issues)
      'slate': getPackageDir('slate'),
      'slate-react': getPackageDir('slate-react'),
      'slate-dom': getPackageDir('slate-dom'),
    };
    return config;
  },
  async headers() {
