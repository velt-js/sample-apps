...
import { teamLists, teamSections } from './constants'
import { SearchIcon, ListIcon, CollapseIcon } from './icons'
...
export default function TaskListSidebar({
  ...
  const filteredSections = teamSections.map(section => ({
    ...section,
        ...
        {filteredSections.map(section => (
          <div key={section.header} className="flex flex-col gap-0.5">
            ...
            {section.items.map(itemId => {
              const list = teamLists.find(l => l.id === itemId)!
              const isSelected = itemId === selectedListId
              return (
                <button
                  key={itemId}
                  ...
                  onClick={() => onSelectList(itemId)}
                >
                  <ListIcon size={16} color={isSelected ? 'rgb(254,255,255)' : 'var(--task-text)'} />
                  <span
                    className="text-xs"
                    style={{
