...
const nextConfig = {
  transpilePackages: ['@veltdev/react', '@veltdev/crdt-react', '@veltdev/crdt'],
  webpack: (config) => {
    // Resolve packages to single instances to prevent duplicate package issues
    config.resolve.alias = {
      ...config.resolve.alias,
      // Ensure single instances of Yjs/CRDT packages (matches Vite dedupe config)
      'yjs': getPackageDir('yjs'),
      'y-protocols': getPackageDir('y-protocols'),
