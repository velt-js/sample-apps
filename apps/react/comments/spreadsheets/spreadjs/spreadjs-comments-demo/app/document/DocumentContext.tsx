...
export function useCurrentDocument(): CurrentDocument {
  ...
  useEffect(() => {
    ...
    if (docId) {
      ...
      setDocumentId(docId);
      localStorage.setItem('spreadjs-comments-demo-document-id', docId);
    } else {
      // 2. Check localStorage for existing document
      const stored = localStorage.getItem('spreadjs-comments-demo-document-id');
      if (stored) {
        docId = stored;
      } else {
  ...
  return useMemo(
    () => ({
      ...
      documentName: "SpreadJS Workbook",
    }),
    [documentId]
  );
}
