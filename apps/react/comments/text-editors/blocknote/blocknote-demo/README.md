...
- Mock user data simulates multi-user scenarios for testing collaboration
...
**BlockNote Editor** (`components/document/BlockNote Component/`)
- Main editor component with Velt CRDT integration
- Configured with Mantine UI components
- Custom styling to match application theme

### Velt CRDT Integration

The core integration uses the `useVeltBlockNoteCrdtExtension` hook:
...
**How it works:**
1. The hook initializes a Yjs document for CRDT synchronization
2. It connects to Velt's WebSocket backend for real-time updates
3. BlockNote's collaboration config is automatically configured with Yjs bindings
...
Velt comments are configured for inline text commenting:
...
1. User selects text in the BlockNote editor
2. User clicks the comment button or uses keyboard shortcut
3. Velt captures the selection context and creates an annotation
4. Comment popover appears for user to type their comment
5. Comment is saved with positional metadata
6. All collaborators see the comment bubble instantly

**Comment Rendering:**
- Comments appear as inline bubbles near the annotated text
- Clicking a bubble opens the comment thread
- Comments persist across editor content changes
- Position is tracked relative to block structure

### Header Integration

The header (`components/header/header.tsx`) contains all Velt tools:
1. **Velt Logo** - Application branding
2. **VeltPresence** - Show online users with avatars
3. **VeltSidebarButton** - Toggle comments sidebar
4. **VeltCommentTool** - Global comment tool
5. **VeltNotificationsTool** - Notifications panel with tabs

### UI Customization

Velt components are customized using wireframes in `components/velt/ui-customization/`:
- **VeltCommentBubbleWf** - Custom styling for comment bubbles
- **VeltCommentToolWf** - Branded comment tool button
- **VeltNotificationsToolWf** - Themed notification panel
- **VeltSidebarButtonWf** - Custom sidebar toggle button
- **Custom CSS** - Additional styling via `styles.css`
